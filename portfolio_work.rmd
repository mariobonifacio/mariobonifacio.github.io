...
fly_wide <- read_csv(file = 'flights_wide.csv', trim_ws = T) %>%
  ...
  group_by(lat, lon) %>%
  summar
  ggplot(aes(lon, lat, color = dep_delay)) +
  geom_point() +
...
corr_list %>%
  mutate(avg = abs(ontime_cor * lndelay_cor),
         monthish = mdy(paste(floor(x / 365 * 12), x / 365 * 12 - floor(x / 365 * 12), 2017))) %>%
...
for(i in 1:23) {
  ...
  corr_list[[i * 100 + j, 3]] <- round(cor(
    abs(interval(start = ymd_hm(paste(2017,
                      month(test_17$time_hour),
                      day(test_17$time_hour),
                      i,
                      j)),
...
%>%
  glm(formula = on_time ~ diff_121day + diff_2029min + wkday, family = binomial) %>%
  summary()
