...
grob_trend <- proj_data %>%
  ...
  labs(title = "Deaths per week", x = NULL)
```
...
grob_case <- proj_data %>%
  ...
  labs(x = NULL, y = "Case increase per thousand", title = str_c(top_n(clean_data[static_data, on = "state"][wk_ending == data_date], 1, wt = positiveIncrease / pop)[,state], " leads in weekly new cases,\nbut some states may undercount by a factor of ", round(undercount, 2))) +
  coord_flip() +
  theme(legend.position = "none")
```
...
grob_death_week <- proj_data %>%
  ...
  labs(x = NULL, y = "Weekly deaths per million", title = str_c(top_n(clean_data[static_data, on = "state"][wk_ending == data_date], 1, wt = deathIncrease / pop)[,state], " sits atop weekly COVID-19 death rate"), caption = ) +
  theme(legend.position = "none") +
  coord_flip()
```
...
```{r}
grid.arrange(grob_case, grob_death_week, grob_trend, grob_cum, ncol = 2)
```
