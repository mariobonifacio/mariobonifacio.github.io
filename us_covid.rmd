...
proj_data <- clean_data[coeff_tbl][
  is.na(lead3_wk_ending), c("lead3_deathIncrease", "lead3_wk_ending", "death_proj") := list(round(est_casepos_case * positiveIncrease + est_casepos_pos * lag1_pos, 0), wk_ending + weeks(3), TRUE)][
    is.na(death_proj), death_proj := FALSE][
      ,truecase_increase := est_pos * lag1_pos / est_casepos_case][
...
```{r}
proj_data %>%
  filter(wk_ending > today() - months(7)) %>% 
  group_by(wk_ending, death_proj) %>% 
  summarize(deathIncrease = sum(deathIncrease)) %>% 
  ggplot(mapping = aes(x = wk_ending, y = deathIncrease, fill = death_proj)) +
  theme_bw() +
  geom_col(color = "dodgerblue", size = 1) +
  scale_fill_discrete(type = c("black", "darkgray")) +
...
proj_data %>%
  ...
  filter(wk_ending == data_date,
         truecase_increase > quantile(positiveIncrease, .8) |
           state %in% c("AZ", "CA", "TX", "NY", "FL")) %>% 
  ggplot(mapping = aes(x = reorder(x = state, X = desc(positiveIncrease)))) +
  ...
  labs(x = NULL, y = "Case increase per thousand", title = str_c(top_n(clean_data[static_data, on = "state"][wk_ending == data_date], 1, wt = positiveIncrease / pop)[,state], " leads the nation in weekly positive increase, but some states may\nbe undercounting by as much as a factor of ", as.integer(undercount)))
```
...
proj_data %>%
  ...
  ggplot(mapping = aes(x = reorder(x = state, X = desc(lead3_deathIncrease)))) +
  ...
  labs(x = NULL, y = "Death increase per million")
```
...
proj_data %>%
  ...
  filter(wk_ending == data_date,
         death > quantile(death, .95, na.rm = TRUE) |
           state %in% c("AZ", "CA", "TX", "NY", "FL")) %>% 
  ggplot(mapping = aes(x = reorder(x = state, X = desc(death)))) +
  geom_col(mapping = aes(y = death), fill = "white", color = "black", linetype = "dashed") +
  geom_col(mapping = aes(y = death, fill = pop_dens), color = "dodgerblue") +
  labs(x = NULL, y = "COVID-19 deaths per million")
```
