...
lifetime_data %>% 
  ...
  filter(as_of > ymd(20200315) &
           is_ca == "Rest of US") %>% 
  ggplot() +
  geom_col(mapping = aes(x = lifetime_data[as_of > ymd(20200315) & is_ca == "Rest of US", as_of], fill = pos_rate, y = lifetime_data[is_ca == "Rest of US" & as_of > ymd(20200315), case_ch_pm]), size = 1, color = "dodgerblue", span = 0.1) +
  geom_line(mapping = aes(x = lifetime_data[as_of > ymd(20200315) & is_ca == "Calif.", as_of], fill = pos_rate, y = lifetime_data[is_ca == "Calif." & as_of > ymd(20200315), case_ch_pm]), size = 1.5, color = "darkred", span = 0.1) +
  geom_label(mapping = aes(x = min(lifetime_data[as_of > ymd(20200315) & is_ca == "Calif.", as_of]), y = min(lifetime_data[is_ca == "Calif." & as_of > ymd(20200315), case_ch_pm]), label = "Calif."), color = "darkred") +
  theme_bw() +
  theme(legend.position = "none") +
  labs(x = "Date", y = "New cases per million per week", title = "U.S. cases are 200% higher than they've ever CA's are double that number", subtitle = str_c("Through ", max(lifetime_data$as_of)))
...
all_data %>%
  ...
  arrange(desc(death_pm)) %>%
  slice_max(order_by = death_pm, n = 10) %>% 
  ggplot(mapping = aes(x = reorder(code, desc(death_pm)), y = death_pm, fill = Density)) +
  geom_col(color = "dodgerblue") +
  scale_fill_gradient2(low = "white", mid = "dodgerblue", high = "white") +
  labs(x = "State", y = "COVID-19 deaths per million residents", fill = "population\ndensity", title = "States with highest COVID-19 death rates may be dense or sparse", subtitle = str_c("COVID-19 deaths per million residents as of ", max(my_data$as_of, na.rm = TRUE))) +
  theme_bw() +
  geom_label(mapping = aes(label = death), fill = "lightgray")
...
weekly_data %>% 
  ...
  theme_bw() +
  theme(legend.title = element_blank()) +
  labs(x = "Date", y = "Deaths per million per week", title = "Weekly deaths remain high in Plains and Midwest states", subtitle = str_c("Week ending ", max(weekly_data$wk_ending))) +
...
weekly_data %>% 
  ...
  theme_bw() +
  theme(legend.title = element_blank()) +
  labs(x = "Date", y = "cases per million per week", title = "States with high weekly cases are spread across the country", subtitle = str_c("Week ending ", max(weekly_data$wk_ending))) +
  scale_fill_brewer(palette="Dark2")
