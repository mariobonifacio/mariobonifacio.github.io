...
tot_data %>% 
  ...
  ggplot() +
  geom_col(mapping = aes(x = reorder(code, X = desc(death_pm)), y = death_pm, fill = Density)) +
...
```{r impute_cases, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
c2t_data <- cor_data %>%
  ...
  filter(as_of > mdy(05012020),
         !is.na(death_ch_lead_3wk),
         between(pos_rate, 0.005, 0.50)) %>%
  group_by(code) %>% 
  slice_min(order_by = pos_rate, n = 30) %>% 
  data.table()
  ...
  c2t_coeff <- rbind(c2t_coeff,
                     data.table(code = i,
                                coeff = lm(data = c2t_data[code == i,],
                                           formula = death_ch_lead_3wk ~ 0 + c2t)[[1]][[1]]))
}

tmp_model <- c2t_data %>% 
  inner_join(c2t_coeff) %>% 
  mutate(case_ch_model = 200 * death_ch_lead_3wk / coeff) %>% 
  data.table()

tmp_model %>% 
  ggplot(mapping = aes(x = as_of)) +
  geom_smooth(mapping = aes(y = case_ch_model), color = "gray", span = 0.0001, alpha = 0, linetype = "dashed") +
  geom_smooth(mapping = aes(y = case_ch), span = 0.0001, alpha = 0)
  # ylim(0,max(tmp_model[,case_ch_model], na.rm = TRUE))
...
all_data %>%
  ...
  scale_colour_discrete(type = c("gray", "black"))
...
weekly_data %>% 
  ...
  theme_bw() +
  theme(legend.title = element_blank()) +
  labs(x = "Date", y = "Deaths per million per week", title = "SD remains atop death rank", subtitle = str_c("Week ending ", max(weekly_data$wk_ending))) +
