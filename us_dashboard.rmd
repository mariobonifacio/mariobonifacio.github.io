...
lifetime_data %>% 
  ...
  geom_col(mapping = aes(fill = pos_rate), size = 1, color = "dodgerblue", span = 0.1) +
  theme_bw() +
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
weekly_proj %>% 
  ggplot(mapping = aes(x = wk_ending)) +
  geom_col(mapping = aes(y = tot_death_ch_proj), na.rm = TRUE, size = 1, fill = "white", color = "darkgray", linetype = "dashed") +
  geom_col(mapping = aes(y = tot_death_ch), size = 1, na.rm = TRUE, color = "dodgerblue") +
  theme_bw() +
  theme(legend.title = element_blank()) +
  labs(x = "Date", y = "Deaths per million", title = str_c("The US saw ",
                                                           sum(weekly_data$death_weekly, na.rm = TRUE),
                                                           " deaths week ending ",
                                                            max(my_data$as_of, na.rm = TRUE),
                                                           "\nand may see ", signif(sum(proj_table[as_of_lead_3wk > max(my_data$as_of, na.rm = TRUE), "death_ch_proj"]), 2), " additional deaths by ", max(my_data$as_of, na.rm = TRUE) + weeks(3)))
