...
just_nordics %>% 
  ...
  geom_point(mapping = aes(color = co_code, size = deaths_pm)) +
  geom_label_repel(mapping = aes(x = x_lbl, y = y_lbl, color = co_code, label = co_code), na.rm = TRUE) +
  theme(legend.position = "none") +
  labs(title = "Sweden has five to 10 times more COVID-19 deaths than other Nordic countries")
```
...
total_oecd %>% 
  ...
  ggplot() +
  geom_col(mapping = aes(x = reorder(co_code, desc(total_deaths_pm)), y = total_deaths_pm)) +
