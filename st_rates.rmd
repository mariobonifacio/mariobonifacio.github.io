...
case_growth %>%
  ...
  labs(x = "state", y = "Weekly cases per million residents", title = stringr::str_c("States with most or least new cases per million"), subtitle = str_c("Week ending", case_growth$wk_ending[1], sep = " "), fill = "pos. rate") +
...
my_data %>% 
  ...
  # theme(legend.position = "none") +
  labs(title = str_c(str_c(pk, collapse = ", "), " infection rate has been lower since May", sep = ""), subtitle = "New daily cases per million residents, split by state", x = "date", y = "new daily COVID-19 cases per million residents")
```
...
my_data %>% 
  ...
  ggplot(mapping = aes(x = dens, y = log_death_pm)) +
                       # , color = hrc, fill = hrc)) +
  geom_label(mapping = aes(x = dens, y = log_death_pm, label = code, fill = NULL, color = hrc)) +
...
my_data %>% 
  ...
  labs(x = "Date", y = "Cumulative cases per million", title = "The U.S. is cresting toward its third wave", subtitle = "Daily COVID-19 cases per million residents") +
  theme(
...
j <- rbind(j,
           ...
           cfr = mean(cor_data[code == k & between(as_of, max(as_of) -14, max(as_of))]$death, na.rm = TRUE) / mean(cor_data[code == k & between(as_of, max(as_of) -14 - i, max(as_of) - i)]$cases, na.rm = TRUE)))
}
  }
...
my_data %>%
  ...
  mutate(as_of_proj = as_of + d_offset,
         wk_proj = if_else(as_of <= max(as_of, na.rm = TRUE) - 7, 1, 2),
         death_pm_proj = case_ch * cfr / pop_2019 * 1E6) %>%
  ungroup() %>% 
  group_by(code, wk_proj) %>%
  summarize(death_pm_proj = sum(death_pm_proj, na.rm = TRUE)) %>% 
  ggplot(mapping = aes(x = wk_proj, y = sum(death_pm_proj))) +
  geom_col()
