...
test_growth %>%
  ...
  filter((tests_weekly_per_mil < quantile(test_growth$tests_weekly_per_mil, probs = 0.2, na.rm = TRUE))
         | (tests_weekly_per_mil > quantile(test_growth$tests_weekly_per_mil, probs = 0.8, na.rm = TRUE))) %>% 
  ggplot() +
...
tot_data %>% 
  ...
  ggplot() +
  geom_col(mapping = aes(x = reorder(code, X = desc(death_pm)), y = death_pm, fill = reg_2)) +
  labs(x = "state", y = "Weekly deaths per million residents", title = stringr::str_c("States with most deaths per million"), subtitle = str_c("As of ", case_growth$wk_ending[1], sep = " ")) +
  theme(legend.title=element_blank(), axis.text.x = element_text(angle = 90, size = 8))
```
...
bestfit_intercept <- lm(formula = temp$log_death_pm ~ temp$Density)[[1]][[1]]
bestfit_slope <- lm(formula = temp$log_death_pm ~ temp$Density)[[1]][[2]]
```
...
my_data %>% 
  ...
  group_by(code) %>%
  mutate(hrc = (t_mgn < 0),
         tier = case_when(Density >= 485 ~ "F",
                          between(Density, 183, 485) ~ "E",
                          between(Density, 69, 183) ~ "D",
                          between(Density, 26, 69) ~ "C",
                          between(Density, 10, 26) ~ "B",
                          Density < 10 ~ "A")) %>% 
  ungroup() %>%
  ggplot(mapping = aes(x = Density, y = log_death_pm)) +
                       # , color = hrc, fill = hrc)) +
  geom_text_repel(mapping = aes(x = Density, y = log_death_pm, label = code, fill = NULL, color = hrc), position = position_jitter()) +
...
my_data %>% 
  ...
  mutate(death_pm = death/Pop * 1000000,
         log_death_pm = log(death_pm, base = 10),
         log_death_pm_model = bestfit_intercept + bestfit_slope * Density,
  ...
  geom_label(mapping = aes(label = code, color = Density, fill = desc(Density))) +
  # geom_smooth(linetype = 3, method = "lm", alpha = 0.1) +
...
my_data %>% 
  ...
  inner_join(pop_dens) %>%  
  mutate(year_mo = month(as_of)) %>% 
  group_by(year_mo, code, dens) %>% 
  summarize(case_ch_pm = 366/12 * mean(case_ch / pop_2019 * 1000000, na.rm = TRUE),
            death_ch_pm = 366/12 * mean(death_ch / pop_2019 * 1000000, na.rm = TRUE),
            is_hrc = sum(t_mgn, na.rm = TRUE) < 0,
            pos_rate = sum(case_ch, na.rm = TRUE) / sum(test_ch, na.rm = TRUE),
            mo_of = median(ymd(str_c("2020", year_mo, "1", sep = "-")))) %>%
  filter(case_ch_pm > 0, death_ch_pm > 0) %>% 
  group_by(year_mo) %>% 
  top_n(n = 5, death_ch_pm) %>%
  ggplot(mapping = aes(x = mo_of, y = death_ch_pm, label = code, fill = dens)) +
  geom_label(position = position_jitter(width = 10, height = 60), segment.color = NA) +
  labs(title = "The states with highest death rates in Dec. 2020 are different from spring",
       subtitle = "Five states with most per capita COVID-19 deaths by month",
       y = "Monthly COVID-19 deaths per million residents",
       x = NULL,
       fill = "pop. per\nsq. mi.") +
  scale_fill_gradientn(colors = c("lightblue", "white", "yellow"))
