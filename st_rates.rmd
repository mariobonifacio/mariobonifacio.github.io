...
my_data %>% 
  ...
  geom_smooth(alpha = 0, span = .6, mapping = aes(color = reg_2)) +
...
case_growth %>%
  ...
  ungroup() %>% 
  ggplot() +
  geom_col(mapping = aes(x = reorder(code, X = desc(cases_weekly_per_mil)), fill = pos_rate, y = cases_weekly_per_mil)) +
