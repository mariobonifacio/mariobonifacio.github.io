...
death_growth %>%
  inner_join(code_names) %>%
  mutate(days_to_double_death = if_else(death_weekly_growth == 0,
                                        0,
                                        round(14 * log(2) / log (1 + death_weekly_growth), 0) / 2)) %>% 
  arrange(desc(days_to_double_death)) %>% 
  ggplot(mapping = aes(x = death_weekly_growth, y = days_to_double_death, color = region)) +
  geom_label(mapping = aes(label = code)) +
  ggplot2::ylim(0, 125) +
  labs(x = "Weekly death growth rate", y = "Days for total number of deaths to double", title = "Two ways to show growth in deaths")
...
test_code <- c("CA", "WV", "TX", "FL", "KY", "TN", "OH", "MS", "GA", "SD", "ND", "AZ")
