...
my_data %>% 
  ...
  ggplot(mapping = aes(x = dens, y = log(death_pm), color = hrc, fill = hrc)) +
  geom_label(mapping = aes(x = dens, y = log(death_pm), label = code, fill = NULL)) +
...
my_data %>% 
  ...
  geom_label_repel(direction = "y", segment.color = NA) +
  theme(legend.position = "none") +
  labs(title = "Five states with most per capita COVID-19 deaths by week",
       y = "Monthly COVID-19 deaths per million residents",
       x = "Date") +
