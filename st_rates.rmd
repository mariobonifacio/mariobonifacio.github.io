...
my_data %>% 
  ...
  labs(x = "state", y = "New COVID-19 cases per million", title = "Sun belt states saw the most new COVID-19 cases in June", subtitle = "New COVID-19 cases per million residents per day from 1 to 30 June 2020 by state") +
  facet_wrap(facets = vars(reorder(code, desc(month_ch))), nrow = 5) +
  theme(axis.text = element_blank(),
        axis.ticks = element_blank(),
        axis.title.x  = element_blank(),
        strip.background = element_rect(fill = "lightgray"),
