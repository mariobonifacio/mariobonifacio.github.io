...
case_growth %>%
  ...
  filter(cases_weekly_per_mil > quantile(case_growth$cases_weekly_per_mil, probs = 0.7, na.rm = TRUE) |
           cases_weekly_per_mil < quantile(case_growth$cases_weekly_per_mil, probs = 0.3, na.rm = TRUE)) %>% 
  group_by(code) %>% 
...
my_data %>% 
  ...
  labs(x = "Date", y = "Cumulative cases per million", title = "The U.S. has crested over its third wave", subtitle = "Daily COVID-19 cases per million residents") +
  theme(
