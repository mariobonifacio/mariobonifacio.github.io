...
my_data %>% 
  ...
  ungroup() %>% 
  group_by(wkday) %>% 
  mutate(wday_case_global = mean(wday_case_local, na.rm = TRUE),
         wday_death_global = mean(wday_death_local, na.rm = TRUE),
         case_ch_adj = case_ch / wday_case_global,
         death_ch_adj = death_ch / wday_case_global) %>%
  ggplot() +
  geom_line(mapping = aes(x = as_of, y = death_ch), color = "gray", size = 3) +
  geom_line(mapping = aes(x = as_of, y = death_ch_adj), color = "black")
