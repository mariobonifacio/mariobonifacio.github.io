...
my_data %>% 
  ...
  inner_join(st_pop_2019) %>%
  group_by(reg_2) %>% 
  filter(code %in% c("AZ", "FL", "SC", "AL", "MS", "CA", "MN", "NY")) %>% 
  filter(as_of > Sys.Date() - months(1)
  ...
  labs(x = "state", y = "New cases per million", title = "Do the appearance of new cases align with protests?", subtitle = str_c("Data from the month ending ", case_growth$wk_ending[1], " suggest they don't", sep = " ")) +
  facet_wrap(facets = vars(reorder(code, reg_2)), nrow = 2) +
