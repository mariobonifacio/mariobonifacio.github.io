...
my_data %>% 
  ...
  labs(x = "state", y = "New cases per million", title = "New cases drop sharply in the northeast while the south rises again", subtitle = str_c("Month ending", case_growth$wk_ending[1], sep = " ")) +
  theme(legend.title=element_blank())
```
...
my_data %>% 
  ...
  labs(x = "state", y = "New cases per million", title = "New cases drop sharply in the northeast, rise in the southwest", subtitle = str_c("Month ending", case_growth$wk_ending[1], sep = " ")) +
...
my_data %>% 
  ...
  geom_vline(xintercept = ymd(20200526) + days(5), color = "dodgerblue", linetype = 2) +
  theme(legend.position = "none") +
  labs(title = str_c("Are", pk, "cases plateauing or even dropping?", sep = " "), subtitle = str_c("cases per day since", min(my_data[code %in% pk, as_of]), sep = " "), x = NULL, y = "COVID-19 daily deaths", caption = "The vertical, dashed blue line marks five days after 26 May, the first day of Minneapolis protests") +
  scale_alpha(range = c(.1, .3))
```
...
my_data %>% 
  ...
  filter(reg_2 %in% c("South", "Midwest", "Northeast", "Southwest", "West Coast")) %>% 
  ...
  theme(legend.title = element_blank()) +
  labs(title = "After mid-May, cases fell in the North from Minnesota to Maine,\nrose on the West Coat, and rose sharply in the sun belt states\nfrom Arizona to Florida", x = NULL, y = "New COVID-19 cases per million residents", caption = "Vertical red and blue lines mark five days after the start of\nMemorial Day weekend and national protests, respectively", subtitle = str_c("Data as of ", max(my_data$as_of)))
```
...
test_code <- c("GA", "WA", "TX", "AR", "CA", "FL", "OK", "MN", "MS", "MI")
