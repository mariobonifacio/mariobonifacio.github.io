...
test_growth %>%
  ...
  labs(x = "state", y = "Weekly tests per million residents", title = stringr::str_c("States with the fewest and most tests per million"), subtitle = str_c("2 weeks ending", death_growth$wk_ending[1], sep = " ")) +
...
my_data %>% 
  ...
  filter(pos_rate < 0.5 | pos_rate > 0.15) %>% 
  ggplot(mapping = aes(x = reorder(code, pos_rate), y = pos_rate, fill = reg_2)) +
  geom_col() +
  geom_label(mapping = aes(label = code)) +
  labs(x = "Date", y = "Share of COVID-19 tests with positive results", title = "Positive rates are increasing in the South and Southwest", subtitle = str_c("Two weeks ending  ", max(my_data$as_of))) +
...
my_data %>% 
  ...
  mutate(week_num = lubridate::week(as_of)) %>% 
  group_by(code) %>%
  summarize(cfr = sum(death_ch) / sum(case_ch), reg_2 = min(reg_2)) %>% 
  ggplot(mapping = aes(x = reorder(code, cfr), y = cfr, fill = reg_2)) +
  geom_col() +
  geom_label(mapping = aes(label = code)) +
  labs(x = "Date", y = "Deaths as a share of confirmed cases", title = "Positive rates are increasing in the South and Southwest") +
  theme(legend.title = element_blank(), axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank())
```
...
my_data %>% 
  ...
  labs(x = "Date", y = "Share of COVID-19 tests with positive results", title = "Positive rates are increasing in the South and Southwest", subtitle = str_c("Two months ending  ", max(my_data$as_of))) +
  theme(legend.title = element_blank())
```
...
death_growth %>%
  ...
  filter(death_weekly_per_mil > quantile(death_growth$death_weekly_per_mil, probs = 0.8) |
           death_weekly_per_mil < quantile(death_growth$death_weekly_per_mil, probs = 0.2)) %>% 
  ggplot() +
  geom_col(mapping = aes(x = reorder(code, X = desc(death_weekly_per_mil)), y = death_weekly_per_mil, fill = reg_2)) +
  labs(x = "state", y = "Weekly deaths per million residents", title = stringr::str_c("States with most deaths per million"), subtitle = str_c("2 weeks ending", case_growth$wk_ending[1], sep = " ")) +
  theme(legend.title=element_blank())
```
...
my_data %>% 
  ...
  filter(us_deaths > 199,
         weekdays(as_of) %in% c("Sunday", "Wednesday")) %>% 
  mutate(since = min(as_of),
         is_day = if_else(weekdays(as_of) == weekdays(my_data[[1,1]]), 0.3, 0.1)) %>% 
...
my_data %>% 
  ...
  filter(us_cases > .000009) %>%
  mutate(since = min(as_of)) %>% 
  ggplot(mapping = aes(x = as_of, y = us_deaths_ch, color = noreast)) +
  geom_smooth(linetype = 0, alpha = .05, mapping = aes(fill = weekdays(as_of))) +
  geom_smooth(alpha = 0, size = 1, linetype = 2) +
  theme(legend.position = "none") +
  labs(title = str_c(str_c(pk, collapse = ", "), " have switched w/others", sep = ""), subtitle = "Deaths per million since one case in 100,000 population had been confirmed", x = "date", y = "COVID-19 new cases")
```
