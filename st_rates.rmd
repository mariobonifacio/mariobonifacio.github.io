...
my_data %>% 
  ...
  filter(lubridate::month(as_of) == 6) %>% 
  mutate(week_num = lubridate::week(as_of)) %>% 
  group_by(code) %>%
  summarize(case_growth = (max(cases) - min(cases)) / min(cases),
            test_growth = (max(tests) - min(tests)) / min(tests)) %>% 
  
  ggplot(mapping = aes(x = case_growth, y = test_growth)) +
  geom_point() +
  geom_smooth(method = "lm", linetype = 2, alpha = 0)
```
...
my_data %>% 
  ...
  ggplot() +
  geom_smooth(mapping = aes(x = as_of, y = case_growth, color = is_clinton), alpha = .2, na.rm = TRUE) +
  labs(x = "state", y = "Growth rate of COVID-19 cases", title = "June 2020 protests appear more likely to have occurred in states won by Trump in 2016", subtitle = "Or the growth in new cases does not align with protests", caption = "Shows rate of growth in cumulative COVID-19 cases from 1 to 30 June 2020") +
