...
my_data %>% 
  ...
  labs(x = "state", y = "New cases per million", title = "New cases drop sharply in the northeast, rise in the south", subtitle = str_c("Month ending", case_growth$wk_ending[1], sep = " ")) +
...
my_data %>% 
  ...
  filter(as_of > ymd(20200331),
         reg_2 %in% c("South", "Midwest", "Northeast", "Southwest")) %>% 
  ...
  geom_vline(xintercept = ymd("2020-05-22") + days(5), color = "red") +
  geom_vline(xintercept = ymd("2020-05-29") + days(5), color = "blue") +
  theme(legend.title = element_blank()) +
  labs(title = "After mid-May, cases fell in the north from Minnesota to Maine\nand rose in sun belt states from Arizona to Florida", subtitle = "New cases per day since 1 April", x = "date", y = "New COVID-19 cases per day", caption = "Vertical red & blue lines mark five days after the start of\nMemorial Day weekend and national protests, respectively")
```
