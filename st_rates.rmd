...
my_data %>% 
  ...
  ggplot(mapping = aes(x = code, y = pos_rate)) +
  geom_col() +
  ...
  facet_wrap(facets = vars(reg_2), nrow = 3)
```
...
my_data %>% 
  ...
  geom_smooth(mapping = aes(y = casesch_post, color = weekdays(as_of)), na.rm = TRUE, method = 'lm', linetype = 'dotted') +
  geom_point(mapping = aes(y = us_cases_ch), na.rm = TRUE) +
  geom_vline(xintercept = ymd(20200716), linetype = 'dashed') +
  theme(legend.position = "none") +
  labs(title = "Did case growth change when reporting switched from CDC to HHS on 16 July?", subtitle = "New U.S. cases per day since 1 July", x = NULL, y = "COVID-19 daily cases") +
  ylim(0, 80000)
```
