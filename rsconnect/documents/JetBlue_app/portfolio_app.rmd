...
library(shiny); library(lubridate); library(tidyverse); library(ggthemes); library(stringr)
...
server <- function(input, output) { ## Define the server code
  output$plot <- renderPlot({
    ...
    my_pred %>%
      ...
      labs(title = str_c('You have a ',
                    round(100*p_on_time, 0),
                    '% chance of flying on time'),
           subtitle = str_c('But if delayed, expect to wait around',
                       round(mdn_pred),
                       'minutes')) +
      ...
      scale_fill_manual(values = c('lightgray','dodgerblue'), aesthetics = 'fill', breaks = c('', 'Best')) +
